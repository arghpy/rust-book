...
fn main() {
    ...
    let number = number.trim().parse().expect("Couldn't convert to integer.");

    println!("Enter the power:");
    io::stdin().read_line(&mut power).expect("Failed to read.");
    let power = power.trim().parse().expect("Couldn't convert to integer.");
